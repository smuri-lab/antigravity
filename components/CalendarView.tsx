                        ...
                        <div className="space-y-3">
                            ...
                            {shiftsForSelectedDay.length > 0 && (
                                <div>
                                    ...
                                    {shiftsForSelectedDay.map(shift => (<div key={shift.id} className="p-3 bg-blue-50 border border-blue-100 rounded-lg flex justify-between items-center"><div><p className="font-semibold text-blue-900">{shift.label || 'Geplante Schicht'}</p><p className="text-xs text-blue-700">{new Date(shift.start).toLocaleTimeString('de-DE', { hour: '2-digit', minute: '2-digit' })} - {new Date(shift.end).toLocaleTimeString('de-DE', { hour: '2-digit', minute: '2-digit' })}</p></div><div className="w-2 h-8 bg-blue-400 rounded-full opacity-30"></div></div>))}
