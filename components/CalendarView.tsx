import { Shift } from '../types'; // Added Shift type

interface CalendarViewProps {
    currentUser: Employee;
    timeEntries: TimeEntry[];
    absenceRequests: AbsenceRequest[];
    shifts?: Shift[]; // Optional to avoid breaking other usages immediately
    customers: Customer[];
    activities: Activity[];
...
// ... (Constants omitted for brevity, keeping existing code)

// ... (CalendarDay and CalendarMonthGrid updated in previous step)
...
return (
    <div className="w-1/3 shrink-0 px-1" style={{ contain: 'content' }}>
        <DayOfWeekHeader />
        <div className="grid grid-cols-7 gap-y-1"> {/* Added gap-y-1 for better spacing with taller cells */}
            {daysData.map((dayItem, index) => {
                ...
                return (
                    <CalendarDay
                        ...
                        hasShift={!isCurrent ? false : shiftsMap.has(dateStr)} // Pass shift status
...
export const CalendarView: React.FC<CalendarViewProps> = (props) => {
    ...
    interface CalendarDayProps {
        ...
        isToday: boolean;
        hasEntry: boolean;
        hasShift: boolean; // NEW: Indicator for planned shift
    ...
    const CalendarDay: React.FC<CalendarDayProps> = React.memo(({
        ...
        let containerClasses = 'relative h-14 flex flex-col items-center justify-start pt-1'; // Increased height slightly
        if (isCurrentMonth) {
        ...
        return (
            <div onClick={handleDayClick} className={containerClasses}>
                ...
                <div className="flex flex-col items-center gap-0.5 mt-0.5 w-full px-2">
                    {/* Planned Shift Bar */}
                    {hasShift && !absenceType && (
                        <div className="w-full h-1 bg-blue-300 rounded-full"></div>
                    )}

                    {/* Actual Time Entry Dot */}
    ...
    interface CalendarMonthGridProps {
        ...
        shiftsMap: Map<string, boolean>; // NEW
        ...
        const { entriesMap, entriesDataMap, shiftsMap, shiftsDataMap, absencesMap, holidaysMap } = useMemo(() => {
            ...
            for (let i = 0; i < absenceRequests.length; i++) {
                ...
                while (loopDate <= endDate && safety < 366) {
                    ...
                    const dStr = `${y}-${m < 10 ? '0' : ''}${m}-${d < 10 ? '0' : ''}${d}`;

                    const existing = aMap.get(dStr);
                        ...
                        <Card>
                            <div className="space-y-4">
                                ...
                                {shiftsForSelectedDay.length > 0 && (
                                    <div className="space-y-2">
                                        ...
                                        {shiftsForSelectedDay.map(shift => (
                                            <div key={shift.id} className="p-3 bg-blue-50 border border-blue-100 rounded-lg flex justify-between items-center">
                                                <div>
                                                    <p className="font-semibold text-blue-900">{shift.label || 'Geplante Schicht'}</p>
                                                    <p className="text-xs text-blue-700">
                                                        {new Date(shift.start).toLocaleTimeString('de-DE', { hour: '2-digit', minute: '2-digit' })} - {new Date(shift.end).toLocaleTimeString('de-DE', { hour: '2-digit', minute: '2-digit' })}
                                                    </p>
                                                </div>
                                                <div className="w-2 h-8 bg-blue-400 rounded-full opacity-30"></div>
                                            </div>
                                        ))}
                                ...
                                {holidayForSelectedDay && (
                                    <div className="p-3 bg-red-50 rounded-lg border border-red-200 flex items-center">
                                        <div className="w-2.5 h-2.5 bg-red-500 rounded-full mr-3"></div>
                                        <p className="font-semibold text-red-800">{holidayForSelectedDay.name} (Feiertag)</p>
                                    </div>
                                )}
                                {absencesForSelectedDayList.map(absence => {
                                    const d = getAbsenceStyle(absence.type);
                                    const dayPortionText = absence.dayPortion === 'am' ? ' (Vormittags)' : absence.dayPortion === 'pm' ? ' (Nachmittags)' : '';
                                    return (
                                        <div key={absence.id} className={`p-3 rounded-lg border flex items-center ${d.bgClass} ${d.borderClass}`}>
                                            <div className={`w-2.5 h-2.5 ${d.dotClass} rounded-full mr-3`}></div>
                                            <p className={`font-semibold ${d.textClass}`}>{d.label}{dayPortionText}</p>
                                        </div>
                                    );
                                })}

                                {/* ACTUAL VIEW (Time Entries) */}
                                {(entriesForSelectedDay.length > 0) && (
                                    <div>
                                        {shiftsForSelectedDay.length > 0 && <h4 className="text-xs uppercase tracking-wider text-gray-400 font-bold mb-2">Erfasste Zeiten (Ist)</h4>}
                                        <div className="space-y-2">
                                            {entriesForSelectedDay.map(entry => {
                                                const d = (new Date(entry.end).getTime() - new Date(entry.start).getTime()) / 36e5 - (entry.breakDurationMinutes / 60);
                                                return (
                                                    <button key={entry.id} onClick={() => setSelectedEntryId(entry.id)} className="w-full p-3 bg-white rounded-lg border flex justify-between items-center text-left hover:bg-gray-50 transition-colors shadow-sm">
                                                        <div>
                                                            <p className="font-semibold text-gray-800">{activities.find(a => a.id === entry.activityId)?.name || 'N/A'}</p>
                                                            <p className="text-sm text-gray-500">{customers.find(c => c.id === entry.customerId)?.name || 'N/A'}</p>
                                                            <p className="text-xs text-gray-400 mt-1">{new Date(entry.start).toLocaleTimeString('de-DE', { hour: '2-digit', minute: '2-digit' })} - {new Date(entry.end).toLocaleTimeString('de-DE', { hour: '2-digit', minute: '2-digit' })}</p>
                                                        </div>
                                                        <p className="font-bold text-lg text-green-600">{formatHoursAndMinutes(d, timeFormat)}</p>
                                                    </button>
                                                );
                                            })}
                                        </div>
                                ...
                                {entriesForSelectedDay.length === 0 && absencesForSelectedDayList.length === 0 && !holidayForSelectedDay && shiftsForSelectedDay.length === 0 && (
                                    <p className="text-center text-gray-500 py-4">Keine Einträge für diesen Tag.</p>
                                )}
                            </div>
                        </Card>
                    </div>
