...
export const EmployeeMultiSelectModal: React.FC<MultiSelectModalProps> = ({ isOpen, onClose, onApply, employees, items, selectedEmployeeIds, selectedItemIds, title = "Mitarbeiter auswählen" }) => {
  ...
  return ReactDOM.createPortal(
    <div className={`fixed inset-0 flex items-center justify-center z-[300] p-4 transition-colors duration-300 ${isClosing ? 'animate-modal-fade-out' : (isVisible ? 'animate-modal-fade-in' : 'bg-transparent')}`}
      onClick={(e) => {
        e.stopPropagation();
        handleClose();
      }}>
