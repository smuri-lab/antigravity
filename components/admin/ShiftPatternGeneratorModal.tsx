...
export const ShiftPatternGeneratorModal: React.FC<ShiftPatternGeneratorModalProps> = ({ 
    ...
    const handleGenerate = () => {
        ...
        selectedEmployeeIds.forEach(empId => {
            ...
            let patternIdx = 0; // Always start pattern at index 0 on StartDate

            while (currentDate <= end) {
                const template = pattern[patternIdx];
            ...
            <Card className={`w-full max-w-3xl relative max-h-[90vh] flex flex-col ${isClosing ? 'animate-modal-slide-down' : 'animate-modal-slide-up'}`} onClick={e => e.stopPropagation()}>
                ...
                <div className="mb-6 border-b pb-4">
                    <h2 className="text-xl font-bold flex items-center gap-2">
                        ...
                        Schicht-Automatik (Rotation)
                    </h2>
                    <p className="text-sm text-gray-500 mt-1">Definieren Sie eine Abfolge von Schichten, die sich automatisch wiederholt.</p>
                </div>
                ...
                <div className="flex-grow overflow-y-auto space-y-6 pr-2">
                    ...
                    <div className="space-y-2">
                        <div className="flex justify-between items-end">
                            <label className="block text-sm font-medium text-gray-700">Muster definieren ({pattern.length} Tage Zyklus)</label>
                        ...
                        <div className="flex flex-wrap gap-2 items-start bg-gray-50 p-4 rounded-lg border border-gray-200">
                                ...
                                <div key={index} className="relative group">
                                    <div className="flex flex-col items-center gap-1">
                                        ...
                                        >
                                            {template ? (
                                                <>
                                                    <span className="font-bold text-sm truncate w-full text-center px-1">{template.name}</span>
                                                    <span className="text-[10px] opacity-75">{template.startTime}-{template.endTime}</span>
                                                </>
                                            ) : (
                                    ...
                                    {/* Template Selection Popover (Simplified inline for now, ideally a portal) */}
                                    {isTemplateSelectOpen.isOpen && isTemplateSelectOpen.index === index && (
                                        <div className="absolute top-full left-0 mt-2 w-48 bg-white rounded-lg shadow-xl border border-gray-200 z-50 p-2 max-h-60 overflow-y-auto">
                                            <div className="text-xs font-bold text-gray-400 mb-2 px-2">Vorlage wählen</div>
                                            <button 
                                                onClick={() => setPatternItem(index, null)}
                                                className="w-full text-left px-2 py-1.5 hover:bg-gray-100 rounded text-sm text-gray-600 mb-1"
                                            >
                                                Frei / Keine Schicht
                                            </button>
                                            {templates.map(t => (
                                            ...
                                            <div className="border-t mt-1 pt-1">
                                                <button onClick={() => setIsTemplateSelectOpen({index: -1, isOpen: false})} className="w-full text-center text-xs text-gray-400 hover:text-gray-600 py-1">Abbrechen</button>
                                            </div>
                                        </div>
                                    )}
                                </div>
                            ...
                            <button 
                                ...
                                className="w-8 h-16 mt-5 rounded-lg border-2 border-dashed border-gray-200 flex items-center justify-center text-gray-400 hover:border-blue-300 hover:text-blue-500 transition-colors"
